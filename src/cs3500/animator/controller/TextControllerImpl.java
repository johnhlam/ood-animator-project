...
/**
 * TextControllerImpl is a class that implements IController. It controls operations between a
 * given IModel, and a text-based IView. It is designed to relay information from a model to a
 * text-based view, so that the view can generate some type of text output. Such output can take
 * the form of a file, or simply a String that is printed.
 */
public class TextControllerImpl implements IController {
  ...
  /**
   * Constructs an instance of this class with the given IView and an IModel.
   * Each of these fields are final, and therefore cannot be changed at a later time. Therefore,
   * each instance of this class can only run with one IView and one IModel. To
   * run with a different set of views/models, a new instance of this class must be created.
   *
   * @param view  is the (text-based) IView that will produce output when necessary.
   * @param model is the IModel that will provide data so that the IView can produce the
   *              appropriate output.
   ...
   * @throws UnsupportedOperationException since this controller does not support being able to
   *                                       set tick rates.
   */
  @Override
